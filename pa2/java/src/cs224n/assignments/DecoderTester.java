 ...
 * @author Gabor Angeli (caching of models)
 */
public class DecoderTester {
  ...
  public static String ZFERTS = "zferts";

  private static interface Function<In>{
    public void apply(In input);
  }

  private static <E> String join(String glue, E... array){
    int k = array.length;
    if(k == 0){ return ""; }
  ...
  private static <E,S extends E> S ensureClass(Class<S> clazz, String[] relevantOptions, Function<E> postCreation){
    S retVal = null;
    try {
      //--Try Cache
      File cacheCand = new File(CACHE_PATH + "/"+clazz.getName()+"."+join(".",relevantOptions));
      if(CACHE_PATH != null && Serializable.class.isAssignableFrom(clazz) && cacheCand.exists()){
        System.out.println("(in cache)");
        ObjectInputStream in = new ObjectInputStream(new FileInputStream(cacheCand));
        retVal = (S) in.readObject();
      //--Else Create
      } else {
        retVal = (S) clazz.newInstance();
        postCreation.apply(retVal);
        try{
          if(CACHE_PATH != null && retVal instanceof Serializable){
            new File(CACHE_PATH).mkdirs();
            cacheCand.createNewFile();
            ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream(cacheCand));
            out.writeObject(retVal);
          }
        } catch(Throwable any){
          System.out.println("WARN: Could not create cache file: " + cacheCand + ": " + any);
          cacheCand.delete();
        }
      }
      return retVal;
  ...
  /* Will create and train the specified language model.
    */
  @SuppressWarnings({"unchecked"})
  public static <E extends LanguageModel> E createLanguageModel(Map<String, String> options, final Collection<List<String>> lmTrainingSentences) throws IOException{
    try {
      System.out.println("Ensuring language model: " + options.get("-lmmodel"));
      Class<E> clazz = (Class<E>) Class.forName(options.get("-lmmodel"));
      String[] relevantOpts = new String[]{options.get("-target"), options.get("-lmsentences")};
      return (E) ensureClass(clazz, relevantOpts, new Function<E>(){
        public void apply(E lm){
          System.out.println("Training language model with "+lmTrainingSentences.size()+" sentences");
          lm.train(lmTrainingSentences);
        }
      });
    } catch (ClassNotFoundException e) {
      throw new RuntimeException(e);
  ...
  @SuppressWarnings({"unchecked"})
  public static <E extends WordAligner> E CreateWordAligner(Map<String,String> options, final List<SentencePair> waTrainingSentencePairs, boolean isReverse){
    try {
      System.out.println("Ensuring word alignmer: " + options.get("-wamodel"));
      Class<E> clazz = (Class<E>) Class.forName(options.get("-wamodel"));
      String[] relevantOpts = new String[]{options.get("-target"), options.get("-source"), options.get("wasentences"), "reverse="+isReverse};
      return (E) ensureClass(clazz, relevantOpts, new Function<E>(){
        public void apply(E wa){
          System.out.println("Training word aligner with "+waTrainingSentencePairs.size()+" sentences");
          wa.train(waTrainingSentencePairs);
        }
      });
    } catch (ClassNotFoundException e) {
      throw new RuntimeException(e);
  ...
  public static void main(String[] args) throws IOException{
    ...
    options.put("-wamodel",     "cs224n.wordaligner.ModelTwoWordAligner");
    options.put("-wasentences", "10000");
    ...
    WordAligner wordaligner = CreateWordAligner(options, waTrainingSentencePairs, false);
    System.out.println("...done\n**********\n");
    ...
    WordAligner reverse_wordaligner = CreateWordAligner(options, reverse_waTrainingSentencePairs, true);
    System.out.println("...done\n**********\n");
    ...
    System.out.println("**********\nCreating / Testing Decoder ...");
		if(options.containsKey("-zferts")){
			ZFERTS = options.get("-zferts");
		}
